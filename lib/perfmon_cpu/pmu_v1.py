...
class v1(PerfmonCpu):
    ...
    def derive_perfmon_metrics(self):
        ...
        '''
        toplevel_df = self.metric_v1(self.raw_df)
        ...
        self.raw_df = pd.concat([self.raw_df, mpki_df, stall_df, missrate_df, instr_df, toplevel_df], axis=1)
        return self.raw_df
    ...
    def metric_v1(self, df_in):

        df_out=pd.DataFrame(index=df_in.index)
        tmp=pd.DataFrame(index=df_in.index)
        tmp['RET/SPEC'] = self._check_div(df_in, 'OP_RETIRED', 'OP_SPEC')
        tmp['STALL/CYCLES'] = self._check_div(df_in, 'STALL_SLOT', 'CPU_CYCLES')
        tmp['STALLF/CYCLES'] = self._check_div(df_in, 'STALL_SLOT_FRONTEND', 'CPU_CYCLES')
        tmp['STALLB/CYCLES'] = self._check_div(df_in, 'STALL_SLOT_BACKEND', 'CPU_CYCLES')

        tmp['BR/CYC'] = (df_in['BR_MIS_PRED']['Values']*4)/df_in['CPU_CYCLES']['Values'] 
        ...
        df_out['Retiring'] = tmp['RETL1']
        df_out['Bad Speculation'] = tmp['BADSPECL1']
        df_out['Frontend Bound'] = 100 * (tmp['STALLF/CYCLES']*(1/8) - tmp['BR/CYC'])
        df_out['Frontend Bound'] = 100 * (tmp['STALLB/CYCLES']*(1/8))
